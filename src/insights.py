...
import pandas as pd
from src.benchmarks import get_rag_status
...
def generate_insights(scorecard: list[dict], df: pd.DataFrame, kpis: dict) -> list[dict]:
    ...
    if rpe_status in ("Red", "Amber"):
        insights.append({
            ...
            "body": (
                f"At ${rpe_now:,.0f}/month per employee, the team is generating below the $15,000 SaaS benchmark. "
        ...
        insights.append({
            ...
            "body": (
                f"Revenue per employee of ${rpe_now:,.0f}/month exceeds the $15,000 SaaS benchmark, "
    ...
    return sorted(insights, key=lambda x: priority[x["status"]])[:5]
