...
from src.benchmarks import build_scorecard, BENCHMARKS, RAG_COLORS, RAG_BG_COLORS
...
<style>
  ...
  .metric-delta-neu {{ color: {AMBER}; font-weight: 600; }}
  ...
  .overview-banner {{
    background: linear-gradient(135deg, {NAVY_LIGHT} 0%, #1a2460 100%);
    border: 1px solid {BORDER};
    border-radius: 14px;
    padding: 24px 28px;
    margin-bottom: 28px;
...
def metric_card(label: str, value: str, sub: str = "", delta: str = "", delta_pos: bool | None = None) -> str:
    ...
    if delta:
        if delta_pos is True:
            delta_class = "metric-delta-pos"
        elif delta_pos is False:
            delta_class = "metric-delta-neg"
        else:
            delta_class = "metric-delta-neu"
        delta_html = f'<div class="metric-sub"><span class="{delta_class}">{delta}</span></div>'
    else:
        delta_html = f'<div class="metric-sub">{sub}</div>' if sub else ""
...
def load_data(company: str) -> pd.DataFrame:
    raw = generate_company_data(company)
    return calculate_kpis(raw)
...
overall_status = "Green" if rag_counts["Red"] == 0 and rag_counts["Amber"] <= 1 else \
...
with col_status:
    ...
    <div style="text-align:right; padding-top:6px;">
      ...
      <div style="display:inline-block; background:rgba({','.join(str(int(overall_color.lstrip('#')[i:i+2], 16)) for i in (0,2,4))},0.15);
...
st.markdown(f'<hr class="styled-divider">', unsafe_allow_html=True)
...
with c1:
    st.markdown(metric_card(
        "Monthly Revenue",
        fmt_currency(kpis["revenue"]),
        delta=f"{rev_delta:+.1f}% MoM",
        delta_pos=rev_delta >= 0,
    ), unsafe_allow_html=True)
with c2:
    st.markdown(metric_card(
        "Cash Balance",
        fmt_currency(kpis["cash_balance"]),
        sub=f"{kpis['runway_months']:.0f} mo runway" if kpis["runway_months"] < 999 else "Profitable",
    ), unsafe_allow_html=True)
with c3:
    st.markdown(metric_card(
        "Gross Margin",
...
with tab1:
    ...
    fig.add_trace(go.Scatter(
        x=df["month_label"], y=df["gross_profit"],
        name="Gross Profit", mode="lines",
        line=dict(color=TEAL, width=2.5),
        fill="tozeroy", fillcolor="rgba(0,200,150,0.06)",
        hovertemplate="$%{y:,.0f}",
    ), row=1, col=1)

    bar_colors = [TEAL if v >= 5 else AMBER if v >= 2 else RED for v in df["revenue_growth_mom"].fillna(0)]
    fig.add_trace(go.Bar(
        x=df["month_label"], y=df["revenue_growth_mom"],
        name="Growth MoM %",
...
with tab2:
    ...
    fig2.add_trace(go.Scatter(
        x=df["month_label"], y=df["cogs"],
        name="COGS", mode="lines",
        line=dict(color=AMBER, width=2, dash="dot"),
        hovertemplate="$%{y:,.0f}",
    ), row=1, col=1)

    fig2.add_trace(go.Scatter(
        x=df["month_label"], y=df["opex"],
        name="OpEx", mode="lines",
    ...
    fig2.add_trace(go.Bar(
        x=df["month_label"], y=df["opex_ratio"],
        name="OpEx Ratio",
...
with tab3:
    ...
    fig3.add_trace(go.Scatter(
        x=df["month_label"], y=df["headcount"],
        name="Headcount", mode="lines+markers",
        line=dict(color=ACCENT, width=2.5),
        marker=dict(size=5, color=ACCENT),
        hovertemplate="%{y} employees",
    ), row=1, col=1)

    rpe_colors = [TEAL if v >= 15000 else AMBER if v >= 10000 else RED for v in df["revenue_per_employee"]]
    fig3.add_trace(go.Bar(
        x=df["month_label"], y=df["revenue_per_employee"],
        name="Rev/Employee",
        marker_color=rpe_colors,
        opacity=0.85,
        hovertemplate="$%{y:,.0f}",
    ), row=2, col=1)
...
with col_score:
    ...
    for row in scorecard:
        ...
        <tr>
          ...
          <td><span class="rag-badge {badge_class}">{row['Status'].upper()}</span></td>
        </tr>
        """
...
with col_radar:
    # Radar chart — normalised 0-1 score per KPI
    ...
    fig_radar.update_layout(
        polar=dict(
            ...
            angularaxis=dict(
                tickfont=dict(size=11, color=TEXT_WHITE),
                gridcolor=BORDER,
                linecolor=BORDER,
            ),
        ),
...
<div style="font-size:13px; color:{TEXT_MUTED}; margin-bottom:20px; line-height:1.6;">
  The following findings are auto-generated based on current KPI performance relative to SaaS SMB industry benchmarks.
  Red and Amber items require management attention; Green findings represent areas of strength.
</div>
""", unsafe_allow_html=True)
...
col_waterfall, col_cash = st.columns(2)

with col_waterfall:
    latest = df.iloc[-1]
    categories = ["Revenue", "COGS", "OpEx", "Net Income"]
    values = [
        latest["revenue"],
        -latest["cogs"],
        -latest["opex"],
    ...
    colors = [ACCENT, RED, AMBER, TEAL if latest["net_income"] >= 0 else RED]
...
with col_cash:
    ...
    fig_cash.add_trace(go.Scatter(
        x=df["month_label"], y=df["cash_balance"],
        name="Cash Balance", mode="lines",
        line=dict(color=ACCENT, width=2.5),
        fill="tozeroy", fillcolor="rgba(79,142,247,0.08)",
        hovertemplate="$%{y:,.0f}",
    ))

    # Burn rate as bars (secondary concept via color)
    burn_colors = [TEAL if v >= 0 else RED for v in df["burn_rate"]]
    fig_cash.add_trace(go.Bar(
        x=df["month_label"], y=df["burn_rate"],
        name="Net Cash Flow",
